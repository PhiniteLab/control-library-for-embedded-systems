...
int main()
{
    ...
    setPIDRoots(&pCont, 0.2, 0.4, 0.5, 0.001);
    //setPIDCoefficients(&pCont, 1, -1, 1, 0.01);

    while (pSys.simulationTime < 10)
    {
        // reference signal generation
        pSys.xRef = 1 + 2 * sin(2 * phi_PI * 1 * pSys.simulationTime);
        float xRefForward = 1 + 2 * sin(2 * phi_PI * 1 * (pSys.simulationTime + pSys.dt));
