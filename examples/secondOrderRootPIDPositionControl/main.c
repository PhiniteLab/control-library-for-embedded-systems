...
int main()
{
    ...
    setPIDRoots(&pCont, 0.1, 0.01, 0.011, 0.01);

    while (pSys.simulationTime < 10)
    {
        // reference signal generation
        pSys.xRef = 10 + 2 * sin(2 * phi_PI * 0.1 * pSys.simulationTime);
        float xRefForward = 10 + 2 * sin(2 * phi_PI * 0.1 * (pSys.simulationTime + pSys.dt));
